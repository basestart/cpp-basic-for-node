...
引用调用 
	...
	形参相当于实参的别名， 对形参的操作实际上是对实参的操作， 被调函数的形式参数虽然也作为局部变量在栈中开辟了内存空间， 但是这时存放的是由主调函数放进来的实参变量的地址， 被调
	函数对形参的任何操作都会被处理成间接寻址， 即通过栈中存放的地址访问主调函数中的实参变量， 正因为如此， 备点函数对形参做的任何操作都影响了主调函数中的实参变量 
